...
- `addRef/decRef` 必须配对。
- Prefab 源资产允许按 `.codex/rules/tyou-dev/prefab-workflow.md` 结构化编辑 JSON；创建优先级：PSD 一键生成 > AI + 精简 MCP Prefab 流程 > 手动拼 > 结构化 JSON 编辑。
- Scene 源资产允许按 `.codex/rules/tyou-dev/scene-workflow.md` 结构化编辑 JSON；Prefab 和 Scene 规则分开读取，不混成一套流程。
