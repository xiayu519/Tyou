...
function createNode(parent, data, parentOffset, parentSize, spriteMap) {
    ...
    if (data.type === 'png') {
        ...
        if (spriteFrameUuid) {
            try {
                ...
                if (cached) {
                    ...
                    cc.assetManager.loadAny(spriteFrameUuid, (err, asset) => {
                ...
                console.warn('[PSD2CCC] Failed to load sprite frame:', data.relativePath, (error === null || error === void 0 ? void 0 : error.message) || error);
            }
        }
    }
        ...
        label.horizontalAlign = (_a = justificationMap[justification]) !== null && _a !== void 0 ? _a : 0;
        label.verticalAlign = 1;
        if (options.fauxBold)
            label.isBold = true;
        if (options.fauxItalic)
            label.isItalic = true;
        ...
        if (options.shadow) {
            ...
            const shadowAlpha = typeof options.shadow.opacity === 'number'
                ...
                : 255;
            label.shadowColor = cc.color(shadowColor.red || 0, shadowColor.green || 0, shadowColor.blue || 0, shadowAlpha);
...
function findSceneNode(uuid) {
    const scene = cc.director.getScene();
    if (!scene)
        return null;
    return findNodeByUuid(scene, uuid);
}
function visitNodeTree(node, visitor) {
    if (!node)
        return;
    visitor(node);
    for (const child of node.children || []) {
...
function getSpriteFrameUuid(spriteFrame) {
    return (spriteFrame === null || spriteFrame === void 0 ? void 0 : spriteFrame._uuid) || (spriteFrame === null || spriteFrame === void 0 ? void 0 : spriteFrame.uuid) || '';
}
function loadSpriteFrame(uuid) {
    ...
    return new Promise((resolve, reject) => {
        cc.assetManager.loadAny(uuid, (err, asset) => {
            if (err || !asset)
                reject(err || new Error(`SpriteFrame not found: ${uuid}`));
            else
                resolve(asset);
        });
    });
...
exports.methods = {
    ...
    collectSpriteFrameRefs(rootUuid) {
        ...
        visitNodeTree(root, (node) => {
            const sprite = node.getComponent && node.getComponent('cc.Sprite');
            const uuid = getSpriteFrameUuid(sprite === null || sprite === void 0 ? void 0 : sprite.spriteFrame);
            if (uuid)
                refs.push(uuid);
        });
        return refs;
    },
    async replaceSpriteFramesInNodeTree(rootUuid, replacementsJson) {
        const root = findSceneNode(rootUuid);
        if (!root)
            return 0;
