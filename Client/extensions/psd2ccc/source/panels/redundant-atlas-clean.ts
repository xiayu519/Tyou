...
function updateSelectedSummary() {
    ...
    if (summary) summary.textContent = totalCount > 0 ? `已勾选 ${selectedCount} / ${totalCount} 张图片` : '没有可清理的图片';
    if (executeButton) executeButton.disabled = isExecuting || selectedCount === 0;
}

function groupAssets(plan: PlanView): Array<{ atlasPath: string; assets: PlanView['assets'] }> {
    const groups = new Map<string, PlanView['assets']>();
    for (const asset of plan.assets) {
        if (!groups.has(asset.atlasPath)) groups.set(asset.atlasPath, []);
...
function groupHtml(group: { atlasPath: string; assets: PlanView['assets'] }, index: number): string {
    return `
...
async function revealAsset(dbPath: string) {
    try {
        ...
        message.send?.('assets', 'twinkle', uuid);
        message.send?.('assets', 'reveal', uuid);
        message.send?.('assets', 'select', uuid);
    } catch (error: any) {
        console.warn('[PSD2CCC] reveal redundant atlas asset failed:', dbPath, error?.message || error);
    }
}
...
function setExecuting(value: boolean) {
    ...
    const executeButton = queryInPanel<HTMLButtonElement>('#execute');
    if (executeButton) executeButton.textContent = value ? '清理中...' : '执行清理';
    updateSelectedSummary();
}
...
function bindEvents() {
    ...
    for (const button of queryAllInPanel<HTMLButtonElement>('.asset-path')) {
        button.addEventListener('click', async () => {
            const dbPath = button.dataset.dbPath || '';
            if (dbPath) await revealAsset(dbPath);
        });
    }
