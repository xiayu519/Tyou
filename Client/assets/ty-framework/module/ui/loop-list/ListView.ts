...
export default class ListView extends Component {
    ...
    @property({
        type: CCInteger,
        tooltip: DEV && '多选时的最大选择数量，为-1时，没有限制',
        visible() {
            return this.selectedMode == SelectedType.MULT;
        },
    })
    public multSelectedMaxNum: number = -1;
            ...
            case SelectedType.SINGLE: {
                ...
                if (t.selectedEvent) {
                    EventHandler.emitEvents(
                        [t.selectedEvent],
                        item,
                        val % this._actualCount,
                        t._lastSelectedId == null ? null : t._lastSelectedId % this._actualCount
                    );
    ...
    set count(val: number) {
        ...
        if (t._virtual) {
            ...
            if (t.frameByFrameRenderNum > 0) {
                //先渲染几个出来
                let len: number = t.frameByFrameRenderNum > t._count ? t._count : t.frameByFrameRenderNum;
                ...
                for (let n: number = 0; n < t._count; n++) {
                    t._createOrUpdateItem2(n);
                }
    ...
    _resizeContent() {
        ...
        switch (t._align) {
            ...
            case Layout.Type.VERTICAL: {
                if (t._customSize) {
                    let fixed: any = t._getFixedSize(null);
                    result = t._topGap + fixed.val + t._itemSize.height * (t._count - fixed.count) + t._lineGap * (t._count - 1) + t._bottomGap;
                } else {
            ...
            case Layout.Type.GRID: {
                ...
                switch (t._startAxis) {
                    case Layout.AxisDirection.HORIZONTAL:
                        let lineNum: number = Math.ceil(t._count / t._colLineNum);
                        result = t._topGap + t._itemSize.height * lineNum + t._lineGap * (lineNum - 1) + t._bottomGap;
                        break;
                    case Layout.AxisDirection.VERTICAL:
                        let colNum: number = Math.ceil(t._count / t._colLineNum);
        ...
        let targetWH: number = t._lack
            ? (t._sizeType ? t._thisNodeUt.height : t._thisNodeUt.width) - slideOffset
            : t.cyclic
    ...
    _onScrolling(ev: Event = null) {
        ...
        if (this._virtual) {
            ...
            let curId: number = 0;
            let endId: number = this._count - 1;
                ...
                switch (this._horizontalDir) {
                    ...
                    case Layout.HorizontalDirection.RIGHT_TO_LEFT: {
                        if (this._customSize) {
                            let fixed: any = this._getFixedSize(id);
                            right =
                                -this._rightGap - (this._itemSize.width + this._columnGap) * (id - fixed.count) - (fixed.val + this._columnGap * fixed.count);
                            let cs: number = this._customSize[id];
                            width = cs > 0 ? cs : this._itemSize.width;
                        } else {
    ...
    update() {
        ...
        if (this._virtual) {
            let len: number =
                this._updateCounter + this.frameByFrameRenderNum > this.displayItemNum ? this.displayItemNum : this._updateCounter + this.frameByFrameRenderNum;
            ...
            if (this._updateCounter < this._count) {
                let len: number =
                    this._updateCounter + this.frameByFrameRenderNum > this._count ? this._count : this._updateCounter + this.frameByFrameRenderNum;
                for (let n: number = this._updateCounter; n < len; n++) {
    ...
    scrollTo(listId: number, timeInSecond: number = 0.5, offset: number = null, overStress: boolean = false) {
        ...
        else if (listId >= t._count) listId = t._count - 1;
