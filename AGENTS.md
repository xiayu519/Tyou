...
本项目是 Cocos Creator 3.8.7 + TypeScript 的 Tyou 客户端框架。本文件是 Codex 项目入口；Codex skills 位于 [.agents/skills/](.agents/skills)，相关任务 Codex 会根据描述自动激活对应 skill；如未自动激活，可显式调用：`$tyou-dev`、`$openspec-propose`、`$openspec-apply-change`、`$openspec-archive-change`、`$openspec-explore`。

具体 Tyou/OpenSpec/UI/资源/Prefab/PSD/Luban/战斗等规范正文统一维护在 [.ai/rules/](.ai/rules)。`.agents/` 只放 Codex skill 入口和路由。

目标：目标明确、按需读取、少上下文、少返工。

## Codex 工作流维护

当 AI 工作流内容发生修改时，必须在同一个 OpenSpec change 中保持这些位置一致：

- 当前会话入口：`AGENTS.md`
- Codex skill 路由：`.agents/skills/*`
- 规则正文：`.ai/rules/`
- 人读说明：`README.md`、`Books/AI-Development-Workflow.md`
- 验收规范：`openspec/specs/`
...
2. **Codex 工作流是否一致**：本次是否修改了 AI 工作流规则、触发、路由、OpenSpec 入口或结束自检？是则检查 `AGENTS.md`、`.agents/skills/*`、`.ai/rules/`、`README.md`、`Books/AI-Development-Workflow.md`、`openspec/specs/` 是否一致。
