...
- **UI 模块** — 栈式管理 + 层级系统 + 模糊背景 + 无限/虚拟列表 + `UIWidget` 子组件 + 右键一键生成 UI 代码
...
检查前缀组件
  ...
  │     → `m_list` 自动修复为 `m_list/content/m_item` 可运行结构
  ▼
生成 UI 代码
  │  ⑥ 右键根节点 → 「🎯 生成UI脚本」
  │     → 自动生成 TypeScript 类（属性绑定 + 事件注册）
  │     → `m_list` 自动绑定 `ListView` 并注册对应 item `UIWidget`
...
4. 脚本自动执行以下操作：
    ...
    - PSD 阶段不做最终资源去重，重复图片先按图层名导出，公共化交给 Cocos 侧「检查公共图集」
    - 图集目录保留 PSD 文件名；普通 PNG 文件名保留图片图层名，不做中文转英文/拼音首字母
    - `.img`、`_9s`、`_scale9` 等工具标签不进入 PNG 文件名和结构 JSON 节点名；同名 PNG 会依次追加 `1`、`2`、`3`
...
| 输出内容 | 路径 | 说明 |
|---------|------|------|
| 切图 PNG | `assets/asset-art/atlas/{psdName}/` | 按 PSD 原名分目录，PNG 按图层原名导出，Cocos 可直接使用 |
...
显式参数支持单轴和零 inset：`_9s_T_B_0_0` 只裁上下并保持原始宽度，`_9s_0_0_L_R` 只裁左右并保持原始高度；任意单侧为 `0` 都不会中止其它有效裁切，四个值全为 `0` 时保留原图尺寸。
...
- 优先复用 `assets/asset-art/atlas/common/` 中已有等价图片；不存在时复制一张代表图进去，并使用来源 PNG 的美术名命名。
- common 目录中同名不同内容已存在时，新 common 文件会依次追加 `1`、`2`、`3`，例如 `按钮.png`、`按钮1.png`。
...
| `m_list` | Tyou `ListView` 标准结构 | 必须有唯一 `m_item` |
...
`m_list` 不是普通 `ScrollView`，而是框架的 Tyou `ListView`。前缀检查会把每个合法列表修复为固定实例结构：
...
- `m_listX` 自动补齐 `cc.ScrollView`、`cc.Mask` 和 `pkg:ListView`，遮罩就在列表根节点上，不额外添加 `cc.Graphics`。
- `content` 自动补齐 `cc.Layout`，并按列表视口顶部对齐。
- `m_itemX` 必须是服务当前列表的唯一模板，自动补齐 `pkg:ListItem`，并绑定到 `ListView.tmpNode`。
- 缺少 `m_item` 或同一个 `m_list` 下存在多个 `m_item` 时，前缀检查会报错并停止把该列表当成已修复结构。
- `m_item` 子树属于 item `UIWidget`，父 UI 只绑定 `m_list`，不会把 `m_item` 内部节点生成到父 UI 字段里。
...
| `m_list` | Tyou ListView | `m_listRank` |
...
#### ListView 与 item Widget 生成

当 UI 中存在 `m_listX/content/m_itemX` 时，生成器会同时处理父 UI 和 item 脚本：

- 父 UI 字段把 `m_listX` 生成为 `ListView`，并在绑定阶段调用 `setItemWidget(ItemX, this)`。
- `m_itemX` 会生成独立 item `UIWidget` 脚本，路径为 `Client/assets/scripts/logic/ui/widget/`，目录不存在时自动创建。
- item 脚本名使用 `Item` + `m_item` 后缀，例如 `m_itemContent` 生成 `ItemContent.ts`。
- 同一次生成发现重复 item 类名时会停止并提示重命名，不会把多个列表静默绑定到同一个 item 类。
- item 脚本由 `ListView` 创建、刷新、回收和释放；父 UI 不直接持有 item 内部节点。
...
`UIImportAll.ts` 还会副作用导入 `./widget/WidgetImportAll`，用于保留动态 Widget 和列表 item Widget 类，避免小游戏构建时被 Tree Shaking 剔除。

> 每次右键生成新 UI 脚本或 Widget 脚本时，相关导入文件会自动重新生成，无需手动维护。

### UIWidget 子组件

`UIWidget` 是 `UIWindow` 下的通用子 UI 生命周期单元，可用于动态加载的子面板、页签、子窗口，也用于 `ListView` 复用的 item。

独立 Widget 预制体脚本通过右键 **「🧱 生成Widget脚本」** 生成：

- 预制体或根节点名字必须包含 `Widget`，例如 `WidgetShop`，脚本名与预制体名一致。
- 脚本生成到 `Client/assets/scripts/logic/ui/widget/`。
- Widget 脚本不写入 `UIName.ts`，也不反向 import 父 UI。
- 生成器会更新 `WidgetImportAll.ts`，并由 `UIImportAll.ts` 统一副作用导入。

父 UI 动态加载和释放 Widget：

```typescript
private _shop: WidgetShop | null = null;

this._shop = await this.loadWidgetByTypeAsync(WidgetShop, this.node, data);

if (this._shop) {
    this.releaseWidget(this._shop);
    this._shop = null;
}
```

如果资源名和类名不一致，可使用 `loadWidgetAsync(WidgetShop, "assetName", parentNode, data)`。父 UI 释放时会统一释放它拥有的子 Widget。

列表 item 是特殊 Widget：它由 `m_item` 生成，类名以 `Item` 开头，并由 `ListView` 按滚动复用生命周期调用 `onRefresh()` / `onRecycle()` / `release()`；普通动态 Widget 默认不生成空的 `onRecycle()`。
...
在 `UIWidget`，尤其是循环列表 item 中，动态图片和 Spine 必须走 owner 生命周期 helper：

```typescript
await this.setSpriteAsync(this._icon, "l_grid1");
await this.setRemoteSpriteAsync(this._avatar, url);
await this.loadSpineAsync(this._spine, "hero_spine");
await this.loadSpineEffectAsync(this._spine, "skill_effect");
```

这些 helper 会把资源登记到当前 `UIBase/UIWidget` owner 上。列表 item 复用、回收到池、列表销毁或父 UI 关闭时，已登记资源会按引用计数调用 `tyou.res.decRef`；异步请求晚到时会校验 owner epoch，旧 index 的图片或 Spine 不会覆盖新 index，也不会留在已回收的 item 上。
...
所有监听器统一管理在同一个 Map 中，`on()` / `once()` 默认优先级为 `NORMAL(50)`，也可通过第四个参数指定优先级。`emit()` 时严格按优先级降序执行，每个监听器独立 try/catch，单个回调异常不影响其他监听器。同一事件递归触发时使用 emit 深度计数，监听移除会延迟到最外层分发完成后处理。
...
// 参数数组形式，适合转发动态参数
tyou.event.emitArray("LEVEL_UP", [newLevel]);
...
每帧批量处理到期计时器，设有安全上限（1000 次/帧）防止死循环。支持暂停、恢复、重启、剩余时间查询。回调中对当前 timer 调用 `restart/resetTimer` 会保留新的调度，销毁后会清理活跃 timer 和对象池，避免回调引用残留。
...
tyou.timer.hasTimer(id);       // 是否仍存在
tyou.timer.getTimerInfo(id);   // 状态快照
tyou.timer.getTimerCount();    // 活跃计时器数量
tyou.timer.getTimerPoolSize(); // 对象池数量
tyou.timer.clearTimerPool();   // 清空对象池
```
...
const name = tyou.storage.get("name", "Guest"); // 缺失或解析失败时返回默认值
tyou.storage.has("userId");
tyou.storage.add("loginCount");
tyou.storage.remove("userId");
tyou.storage.clear();
tyou.storage.clearCache(); // 只清内存缓存，不删 localStorage
```
...
```typescript
// 数值累加
tyou.storage.add("loginCount");        // +1
tyou.storage.add("gold", 100);         // +100

// 按天存储（跨天自动失效）
tyou.storage.setDay("dailyReward", { claimed: true });
const reward = tyou.storage.getDay("dailyReward"); // 隔天返回 null

// 按周存储（跨周自动失效）
tyou.storage.setWeek("weeklyTask", { progress: 3 });
const task = tyou.storage.getWeek("weeklyTask");

// 条件写入回调
tyou.storage.setDay("firstLogin", true, (oldValue) => {
    // 仅当 oldValue 不存在时才写入
});
```

`tyou.storage.ex` 仍保留完整 `StorageEx` 实例入口。未传默认值时，缺失 key 保持旧语义返回 `null`；缓存或持久化 JSON 解析失败返回 `undefined`。
...
每个 target 只允许一个回调（重复注册自动替换）。每帧使用注册表快照执行，遍历期间新增的回调从下一帧开始执行；移除和清空操作会延迟到当前遍历结束后落地。每个回调独立 try/catch。
...
tyou.update.getUpdateTargets();
tyou.update.isUpdating();
...
内部以 `2025-01-01` 为纪元（`TIMESTAMP_EPOCH_2025`），存储时间戳更紧凑。每秒 `schedule` 驱动 `TIME_UPDATE_SECOND` 事件，跨过配置的每日边界时驱动 `TIME_UPDATE_NEW_DAY` 事件。

### API

```typescript
// 同步服务器时间（支持 Unix 秒或毫秒时间戳）
tyou.game.setServerTime(serverTimestamp);

// 获取服务器时间（秒）
const now = tyou.game.getServerTime();
const nowMs = tyou.game.getServerTimeMs();
const localNow = tyou.game.getLocalUnixTime();
...
tyou.game.getServerDayKey();          // 当前服务器日 key
```
...
│   │   │   └── log/               # 日志系统
│   │   ├── module/                # 功能模块
│   │   │   ├── ui/                # UI 模块（UIWindow/UIWidget/ListView）
│   │   │   │   └── loop-list/     # Tyou 无限/虚拟列表运行时
│   │   │   ├── loader/            # 资源模块
│   │   │   ├── audio/             # 音频模块
│   │   │   ├── event/             # 事件模块
│   │   │   ├── timer/             # 计时器模块
...
│   │       ├── widget/                # UIWidget 与 ListView item 脚本（自动生成）
│   │       │   └── WidgetImportAll.ts # Widget副作用导入（自动生成）
