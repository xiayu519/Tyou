---
name: sdd-explore
description: Tyou 轻量 SDD 只读对齐 skill。用于 SDD/spec-driven/explore/grill-me/先聊清楚/需求不确定/对比方案/颗粒度/改动边界，以及新功能、公共契约、跨领域协作、UI/资源/Prefab/Luban/生成链路或存在多种合理实现的任务。只读探索，不实现；输出语义 Change Contract 并等待明确确认。目标与实现明确且语义边界不变的任务不触发。
---
