...
9. 工作流一致：AI 工作流规则、触发、路由、OpenSpec 入口或结束自检变更时，同一个 OpenSpec change 必须检查 `AGENTS.md`、`.agents/skills/*`、`.codex/rules/`、`README.md`、`Books/AI-Development-Workflow.md` 与 `openspec/specs/` 是否一致。
...
| 配置表/Luban | `.codex/rules/tyou-dev/luban-config.md` | `Design/tools/genBin.bat` |
...
| 工作流容错/文档同步 | `.codex/rules/tyou-dev/workflow-recovery.md` | 相关规则 |
...
2. **Codex 工作流是否一致**：本次是否修改了 AI 工作流规则、触发、路由、OpenSpec 入口或结束自检？是则检查 `AGENTS.md`、`.agents/skills/*`、`.codex/rules/`、`README.md`、`Books/AI-Development-Workflow.md` 与 `openspec/specs/` 是否一致。
