...
Tyou 是 Cocos Creator 3.8.7 + TypeScript 客户端框架。本 skill 是 Codex 的 Tyou 开发入口；具体规范正文统一维护在 `.ai/rules/tyou-dev/`。使用本 skill 时保持按需读取，先路由到最小共享规则，再查代码确认。

规则正文只维护在 `.ai/rules/`；OpenSpec artifacts 只维护在 `openspec/`。
...
9. 工作流一致：AI 工作流规则、触发、路由、OpenSpec 入口或结束自检变更时，同一个 OpenSpec change 必须检查 `AGENTS.md`、`.agents/skills/*`、`.ai/rules/`、`README.md`、`Books/AI-Development-Workflow.md` 与 `openspec/specs/` 是否一致。
...
2. **Codex 工作流是否一致**：本次是否修改了 AI 工作流规则、触发、路由、OpenSpec 入口或结束自检？是则检查 `AGENTS.md`、`.agents/skills/*`、`.ai/rules/`、`README.md`、`Books/AI-Development-Workflow.md` 与 `openspec/specs/` 是否一致。
...
在最终回复中用一句话说出这四项的结论（例：“共享规则无需修改；Codex 工作流一致；memory 追加一条 X；OpenSpec change-Y 剩 2 项 task”）。
