...
修改 Cocos `.prefab`、`.prefab.meta` 前必须确认格式和引用关系。开发者已在 `2026-06-09` 明确授权 AI 对项目源 Prefab 执行增删改查：可在明确任务范围内结构化编辑 `Client/assets/**/*.prefab` 和必要的配套 `.prefab.meta`。精确局部修改可用 Direct；跨脚本、资源或 UI 生命周期时使用 Planned。
...
- 特殊需求确实无法合理建模为源 Prefab 或动态子 Prefab 时，程序化临时节点/组件必须在实施前说明用途、范围、生命周期、释放点和验证方式。
