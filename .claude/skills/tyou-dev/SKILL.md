...
这是 Claude Code CLI 专属 skill 壳。具体规范正文统一维护在 `.ai/rules/tyou-dev/`；本 skill 只负责 Claude Code 的 skill 触发和文档路由。使用本 skill 时保持按需读取，先路由到最小共享规则，再查代码确认。

禁止从本 Claude skill 依赖其他 CLI 的专属适配文件；不同 CLI 只共享 `.ai/rules/` 与 `openspec/`。
...
1. 判断 L1-L4。
2. 会话开始处理 L2+ 任务前，先检查 Claude Code memory 或 `.claude/agent-memory/` 中最近相关记录，避免重踩已记录过的问题。
...
在最终回复中用一句话说出这四项的结论（例：“共享规则无需修改；双壳已同步；memory 未新增；OpenSpec change-Y 已归档”）。
