# runtime-ui-lifecycle Specification

## Purpose
Define the Tyou UI runtime lifecycle contract for window loading, instance caching, stack ordering, hide/close behavior, bind-node diagnostics, event cleanup, shared backdrop behavior, and Tip lifecycle safety.
...
- **AND** the resource module handles the late asset through its delayed zero-ref release path
